...
async def test_enhanced_status_manager():
    ...
    try:
        ...
        def mock_callback(message_data):
            ...
            print(f"   📨 Received: {message_data.get('message_type', 'unknown')} - {message_data.get('message', '')[:50]}...")
        ...
        expected_types = ['status', 'agentthinking', 'toolcall', 'contentstream', 'researchfinding']
...
async def test_flow_integration():
    ...
    try:
        ...
        if hasattr(flow, 'status_manager') and hasattr(flow.status_manager, 'send_agent_thinking'):
...
async def test_main_callback_integration():
    ...
    try:
        ...
        def mock_redis_publish(task_id, message_data):
            print(f"   📡 Redis Publish: {message_data.get('message_type', 'unknown')} for {task_id}")
        ...
        test_messages = [
            ...
            {'message_type': 'agentthinking', 'message': 'Agent thinking...', 'agent_name': 'Researcher'},
            {'message_type': 'toolcall', 'message': 'Using web search...', 'tool_name': 'web_search'},
            {'message_type': 'contentstream', 'message': 'Generating content...', 'content_type': 'intro'}
        ]
