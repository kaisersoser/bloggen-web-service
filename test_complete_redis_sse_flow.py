...
def get_jwt_token():
    ...
    try:
        ...
        response = requests.post(f"{BACKEND_URL}/api/auth/jwt-token", 
        ...
        for message in pubsub.listen():
            if message['type'] == 'pmessage':
                ...
                try:
                    ...
                    status = parsed_data.get('status', 'unknown') if isinstance(parsed_data, dict) else 'raw'
                    print(f"📨 [{timestamp}] Redis Channel: {channel} → Status: {status}")
...
def main():
    ...
    sse_thread = threading.Thread(target=monitor_sse_stream, args=(task_id, 180), daemon=True)
    sse_thread.start()
