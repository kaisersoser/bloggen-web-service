...
async def create_enhanced_sse_stream(task_id: str, user, task_manager):
    ...
    async def event_generator():
        ...
        try:
            ...
            if task_manager._redis_manager and hasattr(
                task_manager._redis_manager, "_redis_client"
            ):
                try:
                    ...
                    logger.warning(
                        f"Redis subscription failed, using database polling: {e}"
                    )
                    redis_pubsub = None
            ...
            if redis_pubsub:
                ...
                async for update in listen_to_redis_updates(
                    redis_pubsub, task_id, task_manager
                ):
                    yield update
            else:
                logger.info(f"📊 Using database polling for task {task_id}")
                async for update in poll_database_updates(
                    task_id,
                    task_manager,
                    last_sent_status,
                    last_sent_step,
        ...
        async for message in redis_pubsub.listen():
            ...
            if message["type"] == "message":
                try:
                    ...
                    logger.info(
                        f"📨 Redis update for {task_id}: {redis_data.get('status', 'unknown')}"
                    )
                    ...
                    if updated_task:
                        ...
                        if updated_task.get("status", "").lower() in [
                            "completed",
                            "failed",
                        ]:
                            break
...
async def poll_database_updates(
    task_id,
    task_manager,
    last_sent_status,
    last_sent_step,
