...
class S3CleanupValidator:
            ...
            if success:
                ...
                try:
                    ...
                    response = self.s3_storage.s3_client.list_objects_v2(
                        Bucket=self.s3_storage.bucket_name, MaxKeys=1
                    )
        ...
        try:
            ...
            if final_status:
                ...
                if final_status.status == CleanupStatus.COMPLETED:
                    ...
                    print(
                        f"   ⚠️ Task status: {final_status.status.value} (expected for test)"
                    )
    ...
    async def test_cost_calculation(self) -> bool:
        ...
        try:
            ...
            for case in test_cases:
                ...
                print(
                    f"   {status} {images} images: {calculated_gb:.6f}GB, ${calculated_monthly:.8f}/month"
                )

                if not (gb_match and cost_match):
                    all_passed = False
                    print(
                        f"      Expected: {expected_gb:.6f}GB, ${expected_monthly:.8f}/month"
                    )
    ...
    async def generate_cost_analysis_report(self):
        ...
        try:
            ...
            # Average image size
            avg_image_kb = (
                (total_size_bytes / total_objects / 1024) if total_objects > 0 else 0
            )
            ...
            growth_scenarios = [
                {
                    ...
                    "images_per_month": (
                        total_objects
                        / max(1, (newest_object - oldest_object).days / 30)
                        if oldest_object and newest_object
                        else 0
                    ),
                },
            ...
            for scenario in growth_scenarios:
                ...
                accumulated_images = (
                    images_per_month * 6
                )  # 6 months average with quarterly cleanup
                ...
                print(
                    f"      Steady state: ~{accumulated_images:.0f} images, {storage_gb:.3f}GB"
                )
            ...
            print(
                "   1. Implement quarterly cleanup (current plan) for optimal cost/maintenance balance"
            )
            print("   2. Monitor growth trends and adjust cleanup frequency if needed")
            print(
                "   3. Consider image compression/optimization for high-growth scenarios"
            )
    ...
    async def test_audit_integration(self) -> bool:
        ...
        try:
            ...
            async with EnhancedDatabaseAuditTracker(
                "s3_cleanup_test", user_id="test-user"
            ) as tracker:
                ...
                if tracker.total_cost < 0:
                    print(
                        f"   ✅ Savings properly tracked as negative cost: ${tracker.total_cost:.8f}"
                    )
                else:
                    print(
                        f"   ⚠️ Total cost: ${tracker.total_cost:.8f} (expected negative for savings)"
                    )
...
async def main():
    ...
    parser.add_argument(
        "--test-cleanup", action="store_true", help="Test cleanup queue functionality"
    )
    parser.add_argument(
        "--cost-analysis",
        action="store_true",
    ...
    parser.add_argument(
        "--audit-test", action="store_true", help="Test audit system integration"
    )
    parser.add_argument(
        "--full-test", action="store_true", help="Run complete validation suite"
    )
