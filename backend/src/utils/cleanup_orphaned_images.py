...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("orphaned_cleanup.log"), logging.StreamHandler()],
)
...
class OrphanedImageCleaner:
    ...
    async def get_s3_image_info(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            pages = paginator.paginate(
                Bucket=self.s3_storage.bucket_name, Prefix="hero-images/"
            )
    ...
    async def identify_orphaned_images(
        self,
        ...
        # Categorize images
        by_category = {"has_blog": [], "orphaned": [], "temp_files": [], "invalid": []}
    ...
    async def cleanup_orphaned_images(
        self, orphaned_images: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"🧹 {'DRY RUN: Would delete' if self.dry_run else 'Deleting'} {len(orphaned_images)} orphaned images"
        )
                ...
                try:
                    ...
                    if "Deleted" in response:
                        ...
                        batch_size_bytes = sum(
                            img["size"] for img in batch[:batch_deleted]
                        )
    ...
    async def generate_report(
        self, analysis: Dict[str, Any], cleanup_result: Dict[str, Any]
    ):
        ...
        if analysis["orphaned_count"] > 0:
            ...
            if analysis["oldest_orphan"]:
                print(
                    f"   Oldest orphan: {analysis['oldest_orphan'].strftime('%Y-%m-%d')}"
                )
            if analysis["newest_orphan"]:
                print(
                    f"   Newest orphan: {analysis['newest_orphan'].strftime('%Y-%m-%d')}"
                )
            print()

        print("🧹 CLEANUP RESULTS:")
        print(
            f"   Images {'would be ' if self.dry_run else ''}deleted: {cleanup_result['deleted']:,}"
        )
        ...
        if self.dry_run:
            ...
            if analysis["orphaned_count"] > 0:
                print(
                    f"   Run with --force to delete {analysis['orphaned_count']} orphaned images"
                )
                print(
                    f"   This would save ~${cleanup_result['annual_savings']:.2f} annually"
                )
            else:
                print("   No action needed - no orphaned images found")
            print(
                "   Schedule this script to run quarterly for optimal cost management"
            )
        else:
            print("✅ CLEANUP COMPLETED!")
            if cleanup_result["deleted"] > 0:
                print(
                    f"   Successfully cleaned up {cleanup_result['deleted']} orphaned images"
                )
                print(
                    f"   Ongoing annual savings: ${cleanup_result['annual_savings']:.2f}"
                )
    ...
    async def run_cleanup(self):
        ...
        try:
            ...
            async with EnhancedDatabaseAuditTracker(
                "quarterly_cleanup", user_id="system"
            ) as tracker:
                ...
                if cleanup_result["deleted"] > 0:
                    tracker.track_storage_cleanup(
                        ...
                        status=(
                            "success" if cleanup_result["failed"] == 0 else "partial"
                        ),
                    )
...
async def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Actually delete orphaned images"
    )
    parser.add_argument(
        "--cost-analysis",
        action="store_true",
