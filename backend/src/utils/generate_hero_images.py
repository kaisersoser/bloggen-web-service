...
src_dir = os.path.dirname(script_dir)  # backend/src directory
backend_dir = os.path.dirname(src_dir)  # backend directory
...
class HeroImageGenerator:
    ...
    async def generate_hero_image(self, topic: str, blog_id: str) -> Optional[str]:
        ...
        try:
            ...
            if isinstance(hero_result, str):
                ...
                print(
                    f"  ✅ OpenAI generated image (stored in S3): {hero_url[:80]}..."
                    if hero_url
                    else "  ❌ Failed to extract URL from OpenAI result"
                )
                ...
                print(
                    f"  ✅ OpenAI generated image (stored in S3): {hero_url[:80]}..."
                    if hero_url
                    else "  ❌ No URL in OpenAI result dict"
                )
            ...
            if (
                ...
                try:
                    ...
                    if isinstance(unsplash_res, dict):
                        ...
                        print(
                            f"  ✅ Unsplash generated image: {hero_url[:80]}..."
                            if hero_url
                            else "  ❌ Unsplash also failed"
                        )
    ...
    async def update_blog_hero_image(
        self, conn: asyncpg.Connection, blog_id: str, hero_url: str
    ) -> bool:
        """Update the hero image URL in the database."""
        try:
            if self.dry_run:
                print(
                    f"  🚫 DRY RUN: Would update blog {blog_id} with hero URL: {hero_url[:80]}..."
                )
                return True
    ...
    async def get_blogs_to_process(
        self, conn: asyncpg.Connection, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        ...
        if self.force:
            ...
            condition = "TRUE"
            print(
                "🔄 Force mode: Processing ALL blogs regardless of existing hero images"
            )
        else:
    ...
    async def process_blogs(self, limit: Optional[int] = None):
        ...
        print(
            f"AI Image Generation Enabled: {config.features.enable_hero_image_generation}"
        )
        print("-" * 70)
            ...
            for i, blog in enumerate(blogs, 1):
                ...
                if hero_url:
                    ...
                    if success:
                        ...
                        self.error_count += 1
                        print(
                            f"  💥 ERROR: Generated image but failed to save to database"
                        )
                else:
                    self.error_count += 1
    ...
    def print_summary(self):
        ...
        print(
            f"Mode: {'DRY RUN - No changes made' if self.dry_run else 'LIVE - Changes saved to database'}"
        )
        print("=" * 70)
