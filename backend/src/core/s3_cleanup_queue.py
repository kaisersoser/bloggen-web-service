...
class S3CleanupQueue:
    ...
    async def start(self):
        ...
        self.running = True
        logger.info(f"Starting S3 cleanup queue with {self.max_concurrent_tasks} workers")
    ...
    async def _process_cleanup_task(self, task: CleanupTask, worker_name: str):
        ...
        logger.info(f"Worker {worker_name} processing cleanup for blog {task.blog_id} (attempt {task.attempts}/{task.max_attempts})")
        ...
        try:
            ...
            if failed_urls and task.attempts < task.max_attempts:
                ...
                task.error_message = f"Partial failure: {len(failed_urls)} images failed to delete"
                
                # Re-enqueue for retry with exponential backoff
                retry_delay = min(60 * (2 ** (task.attempts - 1)), 300)  # Max 5 minutes
                logger.warning(f"Blog {task.blog_id} cleanup partial failure, retrying in {retry_delay}s")
                ...
                task.error_message = f"Max attempts reached. {len(failed_urls)} images failed to delete"
                logger.error(f"Blog {task.blog_id} cleanup failed after {task.attempts} attempts")
            else:
                # Complete success
                task.status = CleanupStatus.COMPLETED
                task.error_message = None
                logger.info(f"Blog {task.blog_id} cleanup completed successfully: {successful_deletions} images deleted")
            ...
            if task.attempts < task.max_attempts:
                ...
                logger.warning(f"Blog {task.blog_id} cleanup error, retrying in {retry_delay}s: {e}")
                ...
                # Max attempts reached
                task.status = CleanupStatus.FAILED
                logger.error(f"Blog {task.blog_id} cleanup failed permanently after {task.attempts} attempts: {e}")
                await self._complete_task(task)
    ...
    async def _track_cleanup_metrics(self, task: CleanupTask):
        ...
        try:
            ...
            estimated_size_gb = task.successful_deletions * 0.0002  # 200KB per image in GB
            ...
            # Fallback logging
            logger.info(f"Estimated storage savings for blog {task.blog_id}: "
                       f"{task.successful_deletions} images, ~{estimated_size_gb:.4f}GB, "
                       f"~${monthly_savings:.6f}/month, status={status}")
            
        except Exception as e:
            logger.error(f"Failed to track cleanup metrics for blog {task.blog_id}: {e}")
