...
class RedisMessageBuffer:
    ...
    async def buffer_message(self, task_id: str, channel: str, message_data: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"📦 Buffered {buffered_msg.message_type} message for task {task_id}")
            return True
    ...
    async def flush_buffered_messages(self, task_id: str) -> List[BufferedMessage]:
        ...
        try:
            ...
            logger.error(f"❌ Failed to flush buffered messages for task {task_id}: {e}")
            return []
