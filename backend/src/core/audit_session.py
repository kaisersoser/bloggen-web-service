...
class AuditSession:
    ...
    async def start(self, db_worker: DatabaseWorker):
        ...
        db_worker.queue_operation('start_session', {
    ...
    def track_api_call(self, model: str, input_tokens: int, output_tokens: int,
                      ...
                      cost: Optional[float] = None, db_worker: Optional[DatabaseWorker] = None):
        """Track an API call."""
        # Allow tracking even if session not formally started (for backward compatibility)
        if not self.is_active:
            logger.debug("Session not formally started - auto-activating for API call tracking")
            self.is_active = True
        ...
        logger.info(f"💰 API Call: {model} ({phase}) [{agent_role}] - ${estimated_cost:.4f}")
    
    async def update_blog_id(self, blog_id: str, db_worker: DatabaseWorker):
        """Update the blog ID for this session."""
        self.blog_id = blog_id
        
        db_worker.queue_operation('update_blog_id', {
            'session_id': self.session_id,
            'blog_id': blog_id
        })
    ...
    async def end(self, db_worker: DatabaseWorker):
        ...
        db_worker.queue_operation('end_session', {
            'session_id': self.session_id,
            'end_time': end_time,
            'total_cost': self.total_cost,
            'total_tokens': self.total_tokens,
            'input_tokens': sum(call['input_tokens'] for call in self.logged_calls),
            'output_tokens': sum(call['output_tokens'] for call in self.logged_calls),
