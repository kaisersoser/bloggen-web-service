...
class SupabaseAuditManager:
    ...
    def _get_default_config(self) -> DatabaseAuditConfig:
        ...
        return DatabaseAuditConfig(database_url=database_url, enable_audit=True)
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 201:
                    ...
                    self.logger.info(
                        f"Created audit session: {session_id} ({session_type})"
                    )
                    return session_id
                else:
                    self.logger.warning(
                        f"Failed to create audit session: {response.status_code}"
                    )
                    return None
    ...
    async def log_llm_call(
        ...
        try:
            ...
            pricing = OPENAI_PRICING.get(
                normalized_model,
                OPENAI_PRICING.get("gpt-4o-mini", {"input": 0.0001, "output": 0.0004}),
            )
            input_cost = (input_tokens / 1000) * pricing["input"]
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 201:
                    ...
                    self.logger.warning(
                        f"Failed to log LLM call: {response.status_code}"
                    )
                    return None
    ...
    async def complete_audit_session(
        ...
        try:
            ...
            payload = {"endTime": datetime.utcnow().isoformat(), "status": status}
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.warning(
                        f"Failed to complete audit session: {response.status_code}"
                    )
                    return False
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.warning(
                        f"Failed to get session summary: {response.status_code}"
                    )
                    return None
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.warning(
                        f"Failed to get user cost summary: {response.status_code}"
                    )
                    return {"total_cost": 0, "session_count": 0, "total_tokens": 0}
...
async def create_audit_session(
    session_type: str, user_id: str, blog_id: Optional[str] = None
) -> Optional[str]:
...
async def log_llm_call(
    ...
    **kwargs,
    ...
    return await audit_manager.log_llm_call(
        session_id, model, input_tokens, output_tokens, total_cost, **kwargs
    )
