...
class EnvironmentValidator:
    ...
    def _define_standard_requirements(self):
        ...
        requirements = [
            ...
            # Server settings
            EnvVarRequirement(
                "HOST", required=False, default="localhost", description="Server host"
            ),
            EnvVarRequirement(
            ...
            # URLs and domains
            EnvVarRequirement(
                "FRONTEND_URL", required=False, description="Frontend application URL"
            ),
            EnvVarRequirement(
                "NEXTAUTH_URL",
                required=False,
            ...
            EnvVarRequirement(
                "DATABASE_URL", required=False, description="Database connection URL"
            ),
        ]
        ...
        for name, req in self._requirements.items():
            ...
            if req.required and not value:
                ...
                results["errors"].append(
                    f"Required environment variable '{name}' is not set: {req.description}"
                )
                results["valid"] = False
                continue
            ...
            if not req.required and not value:
                ...
                if not req.default:
                    results["warnings"].append(
                        f"Optional environment variable '{name}' is not set: {req.description}"
                    )
                continue
            ...
            if value and req.choices and value not in req.choices:
                ...
                results["errors"].append(
                    f"Environment variable '{name}' has invalid value '{value}'. Allowed: {req.choices}"
                )
                results["valid"] = False
                continue
    ...
    def get_environment_summary(self) -> str:
        ...
        required_set = [
            name
            for name, info in validation["summary"].items()
            if info["required"] and info["is_set"]
        ]
        required_missing = validation["missing_required"]

        lines.append(
            f"Required Variables: {len(required_set)} set, {len(required_missing)} missing"
        )
        if required_missing:
        ...
        optional_set = [
            name
            for name, info in validation["summary"].items()
            if not info["required"] and info["is_set"]
        ]
        optional_missing = validation["missing_optional"]

        lines.append(
            f"Optional Variables: {len(optional_set)} set, {len(optional_missing)} not set"
        )
