...
class BlogGenRateLimitConfig(RateLimitConfig):
    ...
    phase_configs: Dict[str, Dict[str, Any]] = field(default_factory=lambda: {
        'research': {
...
def get_rate_limit_config_from_env() -> BlogGenRateLimitConfig:
    ...
    config.enable_chunking = os.getenv('ENABLE_REQUEST_CHUNKING', 'true').lower() == 'true'
    config.enable_retries = os.getenv('ENABLE_RETRY_LOGIC', 'true').lower() == 'true'
    
    return config


def get_phase_config(phase: str, config: Optional[BlogGenRateLimitConfig] = None) -> Dict[str, Any]:
    """Get configuration for a specific blog generation phase"""
    if config is None:
        config = get_rate_limit_config_from_env()
    
    return config.phase_configs.get(phase, {
...
if __name__ == "__main__":
    ...
    for phase in ['research', 'content_generation', 'fact_checking', 'finalization']:
        ...
        print(f"{phase}: {phase_config['estimated_tokens']} tokens, {phase_config['preferred_model']} model")
