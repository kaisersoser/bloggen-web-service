...
ERROR_CATALOG = {
    ...
    "INVALID_TOPIC_LENGTH": ErrorResponse(
        ...
        suggested_actions=[
            "Shorten your topic",
            "Use the description field for detailed requirements",
        ],
        retryable=True,
        correlation_id="",
        timestamp="",
    ...
    "CONTENT_FILTER_VIOLATION": ErrorResponse(
        ...
        suggested_actions=[
            "Modify your topic to be more appropriate",
...
def create_auth_error(
    ...
    return create_error_response(
        error_type=error_type, technical_details=details, correlation_id=correlation_id
    )


def create_validation_error(
    field_name: str, details: Optional[str] = None, correlation_id: Optional[str] = None
) -> ErrorResponse:
...
def create_rate_limit_error(
    user_role: str, current_usage: int, limit: int, correlation_id: Optional[str] = None
) -> ErrorResponse:
...
def create_database_error(
    operation: str, details: Optional[str] = None, correlation_id: Optional[str] = None
) -> ErrorResponse:
...
def create_system_error(
    component: str, details: Optional[str] = None, correlation_id: Optional[str] = None
) -> ErrorResponse:
...
def handle_openai_error(
    error: Exception, correlation_id: Optional[str] = None
) -> HTTPException:
    ...
    if "rate limit" in error_str or "quota" in error_str:
        ...
        error_response = create_system_error(
            "openai_timeout", str(error), correlation_id
        )
    elif "api key" in error_str or "authentication" in error_str:
        error_response = create_openai_error(
            str(error), "authentication", correlation_id
        )
    else:
...
def handle_database_error(
    error: Exception, operation: str = "unknown", correlation_id: Optional[str] = None
) -> HTTPException:
...
def handle_validation_error(
    error: Exception, field_name: str = "input", correlation_id: Optional[str] = None
) -> HTTPException:
