...
class ContextAwareAuditCallbackHandler(CustomLogger):
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            ...
            self.logger.info(
                f"🎯 LLM API call intercepted in thread {threading.current_thread().ident}"
            )
            ...
            self.logger.info(
                f"Context: tracker={audit_tracker is not None}, phase={phase}, request={request_id}, user={user_id}"
            )

            if not audit_tracker:
                self.logger.warning(
                    f"No audit tracker in context or global registry for request {request_id}"
                )
                self.logger.warning(
                    f"Available context vars: audit={current_audit_tracker.get(None) is not None}, "
                    f"phase={current_phase.get('none')}, user={current_user_id.get('none')}"
                )
                registry_data = _get_audit_tracker_from_registry()
                self.logger.warning(
                    f"Available in global registry: {registry_data is not None}"
                )
                return
            ...
            self.logger.warning(
                "No context available for API call - this should not happen in FastAPI"
            )
            ...
            if audit_tracker and hasattr(audit_tracker, "log_error"):
                try:
                    error_payload = getattr(
                        getattr(response_obj, "error", None), "message", None
                    )
                    error_message = error_payload or getattr(
                        response_obj, "error", "unknown"
                    )
                    audit_tracker.log_error(  # type: ignore
                        model=model, phase=phase, error=str(error_message)
                    )
                except Exception as track_err:
                    self.logger.debug(
                        f"Failed to record API error in audit tracker: {track_err}"
                    )
        except LookupError:
...
def connect_audit_tracker(audit_tracker):
    ...
    try:
        print(
            f"🔗 Connecting audit tracker in thread {threading.current_thread().ident}"
        )
