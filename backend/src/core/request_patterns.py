...
class PaginationParams:
    ...
    page: int = dataclass_field(default=1)
    page_size: int = dataclass_field(default=DEFAULT_PAGE_SIZE)
    offset: int = dataclass_field(init=False)
...
class SortParams:
    ...
    sort_by: str = dataclass_field(default="created_at")
    sort_order: str = dataclass_field(default="desc")
...
class PaginatedResponse(StandardResponse):
    ...
    total_pages: int = dataclass_field(init=False)
    has_next: bool = dataclass_field(init=False)
...
def validate_enum_field(value: str, field_name: str, allowed_values: List[str]) -> str:
    ...
    if value not in allowed_values:
        raise ValidationError(
            f"{field_name} must be one of: {', '.join(allowed_values)}", field_name
        )
...
def parse_request_params(request_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    pagination = PaginationParams(
        ...
        page_size=safe_int(
            request_data.get("page_size", DEFAULT_PAGE_SIZE), DEFAULT_PAGE_SIZE
        ),
    )
    ...
    return {"pagination": pagination, "sorting": sorting, "filtering": filtering}
...
def create_success_response(
    ...
    return StandardResponse(
        success=True, message=message, data=data, metadata=metadata or {}
    )
...
def create_error_response_standard(
    ...
    return StandardResponse(
        success=False, message=message, errors=errors or [], metadata=metadata or {}
    )
...
class RequestValidator:
    ...
    def validate_blog_generation_request(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        topic = validate_string_length(
            data["topic"], "topic", min_length=5, max_length=200
        )

        # Optional fields with defaults
        length = validate_enum_field(
            data.get("length", "medium"), "length", ["short", "medium", "long"]
        )
    ...
    def validate_user_registration(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        role = validate_enum_field(
            data.get("role", "FREE"), "role", ["FREE", "PREMIUM", "ADMIN"]
        )
    ...
    def validate_blog_update(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "status" in validated_data:
            validated_data["status"] = validate_enum_field(
                validated_data["status"], "status", ["draft", "published", "archived"]
            )
        ...
        if "tags" in validated_data and validated_data["tags"]:
            if isinstance(validated_data["tags"], str):
                ...
                validated_data["tags"] = [
                    tag.strip() for tag in validated_data["tags"].split(",")
                ]
            elif not isinstance(validated_data["tags"], list):
                raise ValidationError(
                    "Tags must be a list or comma-separated string", "tags"
                )
...
class ResponseFormatter:
    ...
    def format_blog_response(blog_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "word_count": (
                len(blog_data.get("content", "").split())
                if blog_data.get("content")
                else 0
            ),
            "reading_time": (
                max(1, len(blog_data.get("content", "").split()) // 200)
                if blog_data.get("content")
                else 1
            ),
        }
    ...
    def format_user_response(user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_login": (
                format_timestamp(user_data.get("last_login"))
...
__all__ = [
    ...
    "create_error_response_standard",
