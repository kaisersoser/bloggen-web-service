...
class CrewAIRateLimitManager:
    ...
    def estimate_crew_execution_tokens(self, crew: Crew, topic: str = "") -> int:
        ...
        logger.debug(
            f"Estimated crew execution tokens: {total} for {len(crew.agents)} agents, {len(crew.tasks)} tasks"
        )
        return total
    ...
    async def execute_crew_with_rate_limiting(
        ...
        async def execute_crew():
            logger.info(
                f"Executing crew for phase '{phase_name}' with estimated {estimated_tokens} tokens"
            )
    ...
    def create_rate_limited_crew(
        self, agents: Sequence[Agent], tasks: Sequence[Task], **crew_kwargs
    ) -> Crew:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(self, *args, **kwargs):
            ...
            async def execute_phase():
                logger.info(
                    f"Executing flow phase '{phase_name}' with {tokens} estimated tokens"
                )
...
class RateLimitedFlow(Flow):
    ...
    async def execute_crew_safely(
        ...
        try:
            return await self._crew_manager.execute_crew_with_rate_limiting(
                crew=crew, inputs=inputs, phase_name=phase_name, max_retries=max_retries
            )
        except Exception as e:
            logger.error(f"Crew execution failed for phase '{phase_name}': {e}")

            # Send status update if callback available
            if hasattr(self, "status_manager") and getattr(
    ...
    def create_rate_limited_crew(
        self, agents: Sequence[Agent], tasks: Sequence[Task], **kwargs
    ) -> Crew:
...
def patch_existing_flows():
    ...
    logger.info(
        "Flow patching configured (requires specific implementation per flow class)"
    )
...
def configure_global_rate_limiting(
    ...
    logger.info(
        f"Configured global rate limiting: {tokens_per_minute} TPM, {requests_per_minute} RPM"
    )
