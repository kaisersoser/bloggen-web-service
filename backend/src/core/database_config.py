    ...
    def validate_database_config(self) -> Dict[str, Any]:
        ...
        for name, path in {
            ...
            try:
                ...
                if not parent_dir.exists():
                    validation['errors'].append(f"Database directory does not exist: {parent_dir}")
                    validation['valid'] = False
                elif not os.access(parent_dir, os.W_OK):
                    validation['errors'].append(f"No write permission for database directory: {parent_dir}")
                    validation['valid'] = False
                else:
        ...
        try:
            ...
            if chroma_dir.exists():
                ...
                chroma_files = list(chroma_dir.glob("*.bin")) + list(chroma_dir.glob("*.sqlite3"))
                if chroma_files:
                    validation['warnings'].append(f"Existing ChromaDB files found: {len(chroma_files)} files")
        except Exception as e:
