...
class S3ImageStorage:
    ...
    def __init__(self):
        ...
        if not all([self.access_key, self.secret_key, self.bucket_name]):
            raise ValueError("Missing AWS S3 configuration. Please check your .env file.")
    ...
    def download_and_convert_image(self, image_url: str, quality: int = 85) -> BytesIO:
        ...
        try:
            ...
            logger.info(f"Converted image to JPEG (quality={quality}), size: {len(jpeg_buffer.getvalue())} bytes")
            return jpeg_buffer
    ...
    def delete_image_by_url(self, s3_url: str) -> bool:
        ...
        try:
            ...
            url_parts = s3_url.split(f"https://{self.bucket_name}.s3.{self.region}.amazonaws.com/", 1)
            if len(url_parts) != 2:
    ...
    def delete_blog_images(self, blog_id: str, content: str | None = None, hero_image_url: str | None = None) -> tuple[int, list[str]]:
        """
        ...
        try:
            ...
            for url in all_urls:
                try:
                    url_parts = url.split(f"https://{self.bucket_name}.s3.{self.region}.amazonaws.com/", 1)
                    if len(url_parts) == 2:
                    ...
                    try:
                        ...
                        if 'Errors' in response:
                            for error in response['Errors']:
                                ...
                                logger.error(f"S3 batch delete error for {error['Key']}: {error['Message']}")
            ...
            logger.info(f"Blog {blog_id} S3 cleanup: {successful_deletions} deleted, {len(failed_urls)} failed")
