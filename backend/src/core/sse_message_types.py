    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            **{
                ...
                if k not in ["message_type", "task_id", "message", "timestamp"]
                and v is not None
            },
        }
...
class ContentStreamMessage(BaseSSEMessage):
    ...
    content_type: str = (
        ""  # 'research', 'outline', 'introduction', 'section', 'conclusion'
    )
    content: str = ""
...
def create_status_message(
    ...
    return StatusMessage(
        task_id=task_id, message=message, status=status, step=step, progress=progress
    )


def create_task_created_message(
    task_id: str, message: str = "Task received and queued"
) -> TaskCreatedMessage:
    return TaskCreatedMessage(task_id=task_id, message=message)


def create_initializing_message(
    task_id: str, phase: str, message: str, progress: float = 0.0
) -> InitializingMessage:
    return InitializingMessage(
        task_id=task_id, message=message, phase=phase, progress=progress
    )


def create_agent_thinking_message(
    task_id: str, agent_name: str, thought: str, message: Optional[str] = None
...
def create_tool_call_message(
    task_id: str, tool_name: str, input_summary: str, agent_name: Optional[str] = None
) -> ToolCallMessage:
...
def create_content_stream_message(
    task_id: str, content_type: str, content: str, is_partial: bool = False
...
def create_research_finding_message(
    task_id: str, finding: str, source: Optional[str] = None
