...
class RLSHelper:
    ...
    @classmethod
    @asynccontextmanager
    async def user_context(cls, user_id: str) -> AsyncGenerator[asyncpg.Connection, None]:
        """
    ...
    async def verify_rls_setup(cls) -> dict:
        ...
        try:
            async with cls.service_context() as conn:
                ...
                logger.info(f"RLS verification complete. Tables checked: {len(rls_status)}")
                return rls_status
...
class DatabaseConnection:
    ...
    @asynccontextmanager
    async def get_user_connection(self, user_id: str) -> AsyncGenerator[asyncpg.Connection, None]:
        """
        ...
        try:
            ...
            logger.debug(f"Database connection established with user context: {user_id}")
            yield conn
    ...
    async def get_service_connection(self) -> AsyncGenerator[asyncpg.Connection, None]:
        ...
        try:
            ...
            logger.warning("Service role database connection established - RLS bypassed")
            yield conn
