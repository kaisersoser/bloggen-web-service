...
class AdvancedRateLimiter:
        ...
        with self._lock:
            if model not in self._token_buckets:
                ...
                self._token_buckets[model] = TokenBucket(
                    ...
                    refill_rate=token_limit / 60.0  # Per second
                )
                ...
                logger.info(f"Created rate limiters for {model}: {token_limit} TPM, {request_limit} RPM")
    ...
    async def acquire(self, model: str, estimated_tokens: int) -> bool:
        ...
        if wait_time > 0:
            logger.info(f"Rate limited for {model}. Waiting {wait_time:.2f}s for {estimated_tokens} tokens")
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    is_retryable = is_rate_limit or any(term in error_str for term in [
                        'timeout', 'connection', 'network', 'server error', '502', '503', '504'
                    ])
            ...
            if last_exception:
                ...
                raise RuntimeError(f"Function failed after {max_retries} retries with unknown error")
...
def rate_limited_api_call(
    ...
    """
    def decorator(func: Callable) -> Callable:
        @exponential_backoff_retry(max_retries=max_retries, rate_limiter=global_rate_limiter)
        @wraps(func)
...
async def safe_openai_call(
    ...
    **kwargs
    ...
    @rate_limited_api_call(model=model, estimated_tokens=estimated_tokens, max_retries=max_retries)
