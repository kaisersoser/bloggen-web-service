...
def test_legacy_api_call():
    ...
    test_data = {"topic": "AI Image Generation Tools in 2025", "current_year": 2025}

    try:
        response = requests.post(
            "http://localhost:5000/api/generate-blog", json=test_data, timeout=120
        )
...
def test_main_api_call():
    ...
    try:
        response = requests.post(
            "http://localhost:8000/generate-blog", json=test_data, timeout=120
        )
