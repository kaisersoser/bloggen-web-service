...
class RedisChannelTester:
    ...
    def analyze_redis_notifications(self):
        ...
        for channel, count in sorted(
            channels.items(), key=lambda x: x[1], reverse=True
        ):
    ...
    def run_redis_test_with_manual_trigger(self):
        ...
        if notification_count and notification_count > 0:
            print(
                f"✅ Redis channels ARE receiving notifications ({notification_count} total)"
            )

            image_count = sum(
                1
                for n in self.notifications_received
                if any(
                    keyword in str(n["data"]).lower()
                    for keyword in ["image", "unsplash", "openai", "dall"]
                )
            )
            ...
            if image_count > 0:
                print(
                    f"✅ Image notifications ARE reaching Redis ({image_count} image events)"
                )
            ...
            print(
                "   → Problem is in Redis pub/sub integration or blog generation not triggered"
            )
