...
def test_frontend_backend_timezone_alignment():
    ...
    try:
        ...
        if backend_response.status_code == 200:
            ...
            if backend_timestamp:
                backend_dt = datetime.fromisoformat(
                    backend_timestamp.replace("Z", "").split(".")[0]
                )
    ...
    try:
        ...
        frontend_response = requests.get(
            "https://localhost:3001/api/auth/jwt-token", verify=False
        )
        ...
        if frontend_response.status_code == 200:
            ...
            if token:
                ...
                try:
                    ...
                    print(
                        f"   Frontend iat: {frontend_iat} -> {datetime.fromtimestamp(frontend_iat)}"
                    )
                    print(
                        f"   Frontend exp: {frontend_exp} -> {datetime.fromtimestamp(frontend_exp)}"
                    )
                except Exception as e:
    ...
    print(
        f"   Current time.time(): {current_time} -> {datetime.fromtimestamp(current_time)}"
    )
    ...
    print(
        f"   JavaScript seconds: {js_seconds} -> {datetime.fromtimestamp(js_seconds)}"
    )
    ...
    print(
        f"   Python times: {datetime.fromtimestamp(python_iat)} -> {datetime.fromtimestamp(python_exp)}"
    )
    ...
    print(
        f"   JavaScript times: {datetime.fromtimestamp(js_iat)} -> {datetime.fromtimestamp(js_exp)}"
    )
