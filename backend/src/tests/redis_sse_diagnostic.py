...
class RedisSSEDiagnostic:
        ...
        try:
            ...
            async for message in pubsub.listen():
                ...
                if message['type'] == 'message':
                    ...
                    msg_type = parsed_data.get('message_type', parsed_data.get('type', 'unknown'))
                    message_text = parsed_data.get('message', str(parsed_data)[:50])
                    print(f"🔴 REDIS [{datetime.now().strftime('%H:%M:%S')}] {channel} → {msg_type}: {message_text}")
    ...
    async def get_jwt_token(self) -> Optional[str]:
        ...
        try:
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=ssl_context)) as session:
                async with session.get(f"{FRONTEND_URL}/api/auth/jwt-token") as response:
    ...
    async def monitor_sse_connection(self):
        ...
        if not token:
            ...
            print("   (SSE requires authentication, but Redis monitoring will continue)")
            return
                    ...
                    async for line in response.content:
                        ...
                        if line.startswith('data: '):
                            ...
                            try:
                                ...
                                msg_type = parsed_data.get('message_type', parsed_data.get('type', 'unknown'))
                                message_text = parsed_data.get('message', str(parsed_data)[:50])
                                print(f"🔵 SSE   [{datetime.now().strftime('%H:%M:%S')}] → {msg_type}: {message_text}")
    ...
    async def generate_comparison_report(self):
        ...
        if self.redis_messages:
            ...
            for msg in self.redis_messages:
                msg_type = msg['data'].get('message_type', msg['data'].get('type', 'unknown'))
        ...
        if self.redis_messages and self.sse_messages:
            redis_types_set = set(msg['data'].get('message_type', msg['data'].get('type', 'unknown')) for msg in self.redis_messages)
            sse_types_set = set(msg['data'].get('message_type', msg['data'].get('type', 'unknown')) for msg in self.sse_messages)
