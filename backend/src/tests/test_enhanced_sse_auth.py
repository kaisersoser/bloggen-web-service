                ...
                async for line in response.content:
                    ...
                    if line.startswith("data: "):
                        try:
                            ...
                            print(
                                f"   📨 Message {message_count}: {data.get('message_type', 'unknown')} - {data.get('message', '')[:50]}..."
                            )

                            # Check for completion
                            if message_count >= 5:  # Get a few messages then exit
                                print(
                                    f"   ✅ Received {message_count} messages - test successful"
                                )
                                break
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("\n💡 Next Steps:")
            print(
                "   1. Frontend now has better error handling for authentication issues"
            )
