...
def test_sse_message_types():
    ...
    try:
        ...
        from core.sse_message_types import (
            TaskCreatedMessage, InitializingMessage, StatusMessage,
            AgentThinkingMessage, ToolCallMessage, ContentStreamMessage,
            ResearchFindingMessage, CompletedMessage, ErrorMessage
        )
        ...
        messages = {
            ...
            'initializing': InitializingMessage("test-123", "Initializing workflow", phase="setup", progress=0.1),
            'agentthinking': AgentThinkingMessage("test-123", "Agent thinking", agent_name="researcher", thought="Analyzing the topic..."),
            'toolcall': ToolCallMessage("test-123", "Using web search tool", tool_name="web_search", input_summary="Searching for AI trends", agent_name="researcher"),
            'contentstream': ContentStreamMessage("test-123", "Generating blog content", content_type="blog_section", content="# AI Trends\n\nThe future...", is_partial=True, word_count=150),
            'researchfinding': ResearchFindingMessage("test-123", "Research finding discovered", finding="AI adoption increased 65%", source="TechReport 2024", relevance_score=0.9),
            'completed': CompletedMessage("test-123", "Blog generation completed", final_content="# Complete Blog\n\nFull content here...", word_count=1200, generation_time=45.2),
            'error': ErrorMessage("test-123", "API rate limit exceeded", error_code="RATE_LIMIT", error_details="API rate limit exceeded", recoverable=True)
        }
        
        for msg_type, message in messages.items():
            msg_dict = message.to_dict()
            print(f"✅ {msg_type}: {msg_dict.get('message_type')} - {msg_dict.get('timestamp')[:19]}")
...
def test_stream_endpoint():
    ...
    try:
        ...
        response = requests.get(f"{BACKEND_URL}/stream/test-task", 
                              timeout=5, verify=False)
