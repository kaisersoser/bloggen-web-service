...
class RealBlogNotificationCollector:
    ...
    def collect_notification(self, notification: Dict[str, Any]):
        ...
        content = message or notification.get(
            "thought", notification.get("tool_name", "N/A")
        )

        logger.info(
            f"{emoji} [{elapsed:6.1f}s] [{self.current_phase or 'init':15s}] {content[:100]}"
        )
    ...
    def print_detailed_summary(self):
        ...
        key_events = [
            ...
            if n["msg_type"] in ["agentthinking", "toolcall", "phase_transition"]
        ]
        ...
        for i, event in enumerate(key_events[:20], 1):  # Show first 20 key events
            ...
            content = (
                ...
                or notification.get("thought")
                or notification.get("tool_name")
                or "N/A"
            )[:80]
        ...
        if self.notifications:
            ...
            progress_notifications = [
                n for n in self.notifications if n["progress"] > 0
            ]
...
if __name__ == "__main__":
    ...
    print(
        f"📊 Notification rate: {len(notifications)/total_duration:.2f} notifications/second"
    )
