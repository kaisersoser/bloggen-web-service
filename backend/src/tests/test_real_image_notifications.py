...
class ImageNotificationAnalyzer:
    ...
    def __init__(self):
        ...
        self.image_event_types = {
            ...
            "unsplash_search",
            "unsplash_fallback",
            "openai_generation_request",
            "openai_generation_response",
            "s3_storage_start",
            "s3_storage_success",
            "image_conversion",
    ...
    def process_notification(self, notification: Dict[str, Any]) -> None:
        ...
        if self._is_image_notification(notification):
            ...
            print(
                f"🖼️  [{elapsed:6.1f}s] IMAGE EVENT: {notification.get('type')} - {notification.get('data')}"
            )
    ...
    def analyze_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "timeline": timeline,
            "image_event_rate": (
                len(self.image_notifications) / total_time if total_time > 0 else 0
            ),
        }
    ...
    def _summarize_event(self, event: Dict[str, Any]) -> str:
        ...
        if event_type == "unsplash_init":
            return (
                f"Unsplash initialized (API key: {data.get('api_key_preview', 'N/A')})"
            )
    ...
    def print_report(self):
        ...
        print(f"=" * 60)
        print(
            f"📊 Total execution time: {analysis['total_execution_time']:.1f} seconds"
        )
...
def test_real_blog_with_image_analysis():
    ...
    try:
        ...
        flow = BlogGenerationFlow(status_callback=analyzer.process_notification)
        ...
        print(
            f"📄 Blog length: {len(str(result.get('final_blog_post', '')))} characters"
        )
