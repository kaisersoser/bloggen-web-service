...
class BlogGenerationRequest:
    ...
    def to_run_inputs(self) -> Dict[str, Any]:
        ...
        return {"topic": self.topic, "current_year": str(self.current_year)}
...
class BlogGenerationService:
    ...
    def generate_blog(
        self, request_data: BlogGenerationRequest
        ...
        try:
            ...
            if hasattr(result, "output") and result.output:
                ...
                return BlogGenerationResponse(success=True, content=content)
            else:
                # If no direct output, try to find generated file
                expected_file_path = self.file_manager.get_blog_file_path(
                    request_data.topic
                )
                ...
                if content:
                    ...
                    return BlogGenerationResponse(
                        success=True, content=content, file_path=str(expected_file_path)
                    )
                else:
            ...
            return BlogGenerationResponse(success=False, error=error_msg)
...
class LegacyFlaskAPI:
    ...
    def run_development_server(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "⚠️  This is the legacy API. Use main.py for new development."
            )
...
def generate_blog():
    ...
    logger.warning(
        "Using deprecated generate_blog function. Use LegacyFlaskAPI class instead."
    )
