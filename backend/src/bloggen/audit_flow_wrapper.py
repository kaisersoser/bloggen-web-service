...
class AuditTrackingFlowWrapper:
    ...
    def __init__(
        self, user_id: str, blog_id: Optional[str] = None, status_callback=None
    ):
        ...
        self.flow = BlogGenerationFlow(
            status_callback=status_callback, user_id=user_id, blog_id=blog_id
        )

        # Audit tracker will be set up during execution
        self.audit_tracker: Optional[EnhancedDatabaseAuditTracker] = None

    async def execute_flow_with_audit(
        self, topic: str, instructions: str = "", current_year: int = 2024
    ):
        """
        ...
        self.audit_tracker = EnhancedDatabaseAuditTracker(
            session_type="blog_generation", user_id=self.user_id, blog_id=self.blog_id
        )

        try:
            async with self.audit_tracker:
                self.logger.info(
                    f"Starting audit-tracked blog generation for user {self.user_id}"
                )

                # Execute the flow (this is synchronous)
                result = await asyncio.get_event_loop().run_in_executor(
                    None, self._execute_flow_sync, topic, instructions, current_year
                )
    ...
    def execute_flow_sync(
        ...
        try:
            ...
            return asyncio.run(
                self.execute_flow_with_audit(topic, instructions, current_year)
            )
        except Exception as e:
            self.logger.error(f"Error in synchronous flow execution: {e}")
            # Fall back to regular flow execution without audit tracking
            self.logger.warning(
                "Falling back to regular flow execution without database audit"
            )
...
async def create_title_audit_session(user_id: str, instructions: str):
    ...
    audit_tracker = EnhancedDatabaseAuditTracker(
        session_type="title_generation", user_id=user_id, blog_id=None
    )
