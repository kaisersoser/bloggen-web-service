    ...
    def _extract_call_info(self, args, kwargs, call_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "messages_preview": [
                    msg.get("content", "")[:100] for msg in messages[-2:]
...
class CrewAILLMInterceptorWrapper:
    ...
    def execute_crew_with_llm_visibility(
        self, crew, phase_name: str = "crew_execution"
    ):
        ...
        logger.info(
            f"🔍 Starting CrewAI execution with LLM interception for phase: {phase_name}"
        )
    ...
    def _handle_llm_event(self, event: Dict[str, Any]):
        ...
        if event_type == "llm_call_start":
            ...
            agent_name = f"{self.current_phase.title()} Agent"
            self._send_agent_thinking(
                agent_name, f"Generated response ({total_tokens} tokens used)"
            )
