...
class ContentIntegrityValidator:
    ...
    def find_content_citing_references(self, content: str, ref_numbers: Set[str]) -> List[str]:
    ...
    def remove_citing_content(self, content: str, problematic_refs: Set[str]) -> str:
        ...
        for paragraph in paragraphs:
            ...
            cites_problematic = any(f'[{ref_num}]' in paragraph for ref_num in problematic_refs)
            
            if not cites_problematic:
                clean_paragraphs.append(paragraph)
            else:
                self.logger.info(f"Removing paragraph citing refs {[r for r in problematic_refs if f'[{r}]' in paragraph]}: {paragraph[:100]}...")
        
        # Reconstruct content
        clean_content = '\n\n'.join(clean_paragraphs)
        return clean_content + '\n\n## References' + references_part

    def remove_invalid_references(self, content: str, analyses: List[ReferenceAnalysis]) -> str:
        ...
        for old_num, new_num in old_to_new_mapping.items():
            content_before_refs = content_before_refs.replace(f'[{old_num}]', f'[{new_num}]')
        
        # Construct final content
        new_references_section = '\n'.join(new_ref_lines)
        final_content = f"{content_before_refs}\n\n## References\n\n{new_references_section}"
        
        return final_content

    def validate_content_integrity(self, blog_content: str) -> Tuple[str, ContentIntegrityReport]:
        """
        ...
        for ref in references:
            ...
            if analysis.action == "remove":
                ...
                self.logger.warning(f"❌ Reference {ref['number']} marked for removal: {issues_str}")
            else:
                self.logger.info(f"✅ Reference {ref['number']} is valid")
        
        # Step 3: Identify problematic reference numbers
        problematic_refs = {a.reference_number for a in analyses if a.action == "remove"}
        self.logger.info(f"🗑️ Removing references: {list(problematic_refs)}")
        
        # Step 4: Find content citing problematic references
        citing_content = self.find_content_citing_references(blog_content, problematic_refs)
        self.logger.info(f"🗑️ Found {len(citing_content)} content elements citing problematic references")
        ...
        self.logger.info(f"✅ Content Integrity Validation Complete: {quality_score:.1f}% content quality")
