...
class URLQualityGate:
    ...
    def find_working_alternative(self, broken_url: str) -> Optional[str]:
        ...
        if clean_domain in self.domain_alternatives:
            ...
            for alt_domain in alternatives:
                ...
                if analysis.is_working:
                    self.logger.info(
                        f"Found working alternative: {alt_url} for {broken_url}"
                    )
                    return alt_url
        ...
        if parsed_url.path and len(parsed_url.path) > 1:
            ...
            if len(path_parts) > 2:
                ...
                if analysis.is_working:
                    self.logger.info(
                        f"Found working simplified URL: {alt_url} for {broken_url}"
                    )
                    return alt_url
    ...
    def fix_broken_url(self, analysis: URLAnalysis) -> URLAnalysis:
        ...
        if alternative:
            ...
            self.logger.info(
                f"Replaced broken URL {analysis.original_url} with {alternative}"
            )
        else:
            ...
            analysis.action_taken = "replaced"
            self.logger.warning(
                f"Using placeholder for unfixable URL {analysis.original_url}"
            )
        ...
        for analysis in analyses:
            if analysis.action_taken == "replaced" and analysis.replacement_url:
                ...
                modified_content = modified_content.replace(
                    analysis.original_url, analysis.replacement_url
                )
                self.logger.info(
                    f"Applied fix: {analysis.original_url} → {analysis.replacement_url}"
                )
    ...
    def run_quality_gate(self, blog_content: str) -> Tuple[str, QualityGateReport]:
        ...
        self.logger.info(
            f"🎯 Quality Gate Complete: {working_count}/{len(urls)} URLs working, {fixed_count} fixed"
        )
...
def format_quality_report(report: QualityGateReport) -> str:
    ...
    for i, analysis in enumerate(report.analysis_results, 1):
        ...
        action = (
            f" → {analysis.action_taken.upper()}"
            if analysis.action_taken != "none"
            else ""
        )
