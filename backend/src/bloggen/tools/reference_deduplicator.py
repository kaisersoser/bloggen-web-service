...
class ReferenceDeduplicator:
    ...
    def extract_references_section(self, content: str) -> Tuple[str, str]:
        ...
        match = re.search(
            self.reference_section_pattern, content, re.DOTALL | re.IGNORECASE
        )
    ...
    def consolidate_duplicates(
        self, duplicate_groups: Dict[str, List[Reference]]
    ) -> List[Reference]:
    ...
    def update_inline_citations(
        self, content: str, reference_mapping: Dict[int, int]
    ) -> str:
    ...
    def deduplicate_references(self, content: str) -> Tuple[str, DeduplicationReport]:
        ...
        references_section, content_without_refs = self.extract_references_section(
            content
        )
        ...
        self.logger.info(
            f"🔍 Found {len(duplicate_groups)} groups of duplicate references"
        )
        ...
        unique_references = [
            ref for ref in references if ref.original_number not in all_duplicated_refs
        ]
        ...
        updated_content = self.update_inline_citations(
            content_without_refs, reference_mapping
        )
        ...
        self.logger.info(
            f"✅ Deduplication complete: {duplicates_removed} duplicates removed"
        )
...
def format_deduplication_report(report: DeduplicationReport) -> str:
    ...
    if report.references_consolidated:
        ...
        for i, ref in enumerate(report.references_consolidated, 1):
            lines.append(
                f"{i:2d}. {ref.title[:60]}{'...' if len(ref.title) > 60 else ''}"
            )
