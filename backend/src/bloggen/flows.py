...
from .tools.url_validation_enforcer import create_validation_enforcer
from .tools.reference_deduplicator import (
...
class BlogGenerationFlow(Flow):
    ...
    def __init__(
        ...
        if config.rate_limit.enabled:
            ...
            logger.info(
                f"Rate limiting enabled: {config.rate_limit.tokens_per_minute} TPM, {config.rate_limit.requests_per_minute} RPM"
            )
        else:
                        ...
                        if loop.is_running():
                            ...
                            def run_in_new_loop():
                                try:
                                    ...
                                    result_val = new_loop.run_until_complete(
    ...
    def _send_periodic_updates_during_execution(
        self, phase_name: str, execution_complete
    ):
        ...
        phase_updates = {
            "research": [
                ...
                (
                    "📚 Searching academic databases and recent publications...",
                    "Research Agent",
                ),
                (
                    "🏢 Gathering industry reports and expert insights...",
                    "Research Agent",
                ),
        ...
        updates = phase_updates.get(
            phase_name,
            [
                (
                    "⚙️ Processing request...",
                    f"{phase_name.replace('_', ' ').title()} Agent",
                ),
                (
                    "🔍 Analyzing content...",
                    f"{phase_name.replace('_', ' ').title()} Agent",
        ...
        try:
            while not execution_complete.is_set():
                if update_index < len(updates):
                    ...
                    logger.info(
                        f"🔄 Sending periodic update {update_index + 1}: {message}"
                    )
                    self.status_manager.send_agent_thinking(
                        agent_name=agent_name, thought=message
                    )
                    update_index += 1
                else:
                    ...
                    logger.info(
                        f"🔄 Sending generic update {cycle_index + 1}: {message}"
                    )
                    self.status_manager.send_agent_thinking(
                        agent_name=agent_name, thought=message
                    )
            ...
            if phase_name == "research":
                ...
                if result_str:
                    ...
                    for paragraph in paragraphs:
                        asyncio.create_task(
                            task_manager.stream_content_paragraph(
                                self.blog_id, paragraph
                            )
                        )
                ...
                if result_str:
                    ...
                    for correction in corrections:
                        asyncio.create_task(
                            task_manager.stream_fact_correction(
                                self.blog_id, correction
                            )
                        )
    ...
    def _auto_generate_topic(self) -> None:
        ...
        if not raw_instructions:
            ...
            logger.warning(
                "Auto-topic generation skipped (no instructions); using fallback 'AI Blog Topic'."
            )
            return
        try:
        ...
        try:
            ...
            for sentence in sentences:
                ...
                if (
                    ...
                    and any(
                        indicator in sentence_lower for indicator in insight_indicators
                    )
                ):
    ...
    def _determine_content_strategy(self, topic: str, research_word_count: int) -> str:
        ...
        try:
            if research_word_count > 1000:
                ...
                word in topic.lower() for word in ["trend", "future", "prediction"]
            ):
    ...
    def _get_content_capabilities(self, tools) -> str:
        ...
        try:
            ...
            return (
                ", ".join(capabilities)
                if capabilities
                else "content generation, structure optimization"
            )
        except Exception:
    ...
    def _extract_content_sections(self, content: str) -> list[str]:
        ...
        try:
            ...
            return (
                ...
                else ["Content sections generated"]
            )
        except Exception:
    ...
    def _analyze_verified_content(self, verified_content: dict) -> dict:
        ...
        try:
            ...
            return {
                ...
                "section_count": 0,
                "paragraph_count": 0,
                "finalization_strategy": "standard finalization",
            }
    ...
    def content_generation_phase(
        ...
        try:
            ...
            research_insights = (
                len(str(research_result).split()) if research_result else 0
            )
            ...
            if draft:
                try:
                    ...
                    draft_text = (
                        str(draft)
                        if hasattr(draft, "__str__")
                        else draft.raw if hasattr(draft, "raw") else ""
                    )
                    ...
                    logger.warning(
                        f"Failed to broadcast content stream: {broadcast_error}"
                    )
    ...
    def content_validation_phase(
        self, content_data: Dict[str, Any]
    ) -> Dict[str, Any]:  # Phase 2.5
        ...
        try:
            ...
            if validation["deprecated_images"] > 0:
                logger.warning(
                    f"⚠️ Found {validation['deprecated_images']} deprecated image sources, cleaning content"
                )
                ...
                if validation["total_images"] > validation["valid_images"]:
                    self._status(
                        "Regenerating images...",
                        step=3,
                        detail="Using proper image tools",
                    )
                    try:
                        # Get content creation tools and agent
                        tools = self.tools_manager.get_content_tools()
                        agent = self.agent_factory.create_content_creator(
                        ...
                        enhanced_content = self._execute(
                            agent, image_task, "image_enhancement"
                        )

                        # Re-validate the enhanced content
                        final_validation = ContentValidator.validate_content(
                            str(enhanced_content)
                        )
                        ContentValidator.log_validation_results(
                            final_validation, "Post-Enhancement"
                        )
            ...
            self._status(
                "Content validation complete", step=3, detail="Ready for fact-checking"
            )
            return {
                **content_data,
                "initial_content": validated_content,
    ...
    def fact_checking_phase(
        self, content_data: Dict[str, Any]
    ) -> Dict[str, Any]:  # Phase 3
        self._require_topic()
        self._begin_phase(
            "fact_checking",
            step=4,
    ...
    def _execute_fact_check_with_validation_loop(
        self, topic: str, tools: list, content: str, enforcement_result
    ) -> Any:
        ...
        while retry_count < max_retries:
            try:
                if retry_count == 0:
                    ...
                    self._status(
                        f"Fact checking with URL validation (attempt {retry_count + 1})",
                        step=4,
                        detail=f"Validating {len(enforcement_result.urls_found)} URLs",
                    )
                    ...
                    self._status(
                        f"Retry fact checking with enhanced validation (attempt {retry_count + 1})",
                        step=4,
                        detail="Enforcing compliance",
                    )
                ...
                logger.info(
                    f"🔒 Executing fact check with validation enforcement (attempt {retry_count + 1})"
                )
                result = self._execute(agent, task, "fact_checking")

                # Check compliance of the result
                is_compliant, compliance_score, compliance_issues = (
                    validation_enforcer.check_validation_compliance(
                        str(result), enforcement_result.urls_found
                    )
                )

                # Store compliance results for audit
                self.flow_state.results[
                    f"validation_compliance_attempt_{retry_count + 1}"
                ] = {
                ...
                if is_compliant:
                    logger.info(
                        f"✅ URL Validation Compliance achieved! Score: {compliance_score:.1%}"
                    )
                    self._status(
                        "URL validation compliance achieved",
                        step=4,
                    ...
                    logger.warning(
                        f"❌ URL Validation Compliance failed. Score: {compliance_score:.1%}"
                    )
                    logger.warning(f"Issues: {compliance_issues}")

                    if retry_count < max_retries - 1:
                        self._status(
                            f"Compliance failed - retry required",
                            step=4,
                            detail=f"Score: {compliance_score:.1%}",
                        )
                        retry_count += 1
                        ...
                        logger.error(
                            f"❌ Maximum retries reached. Final compliance score: {compliance_score:.1%}"
                        )
                        self._status(
                            "Max retries reached - proceeding with partial compliance",
                            step=4,
                            detail=f"Final score: {compliance_score:.1%}",
                        )
                        return result
        ...
        raise RuntimeError(
            "Fact checking with validation loop failed after maximum retries"
        )

    def _create_enhanced_fact_check_task(
        self,
        topic: str,
        enforcement_result,
        is_retry: bool = False,
...
"""
    ...
    def finalization_phase(
        ...
        try:
            ...
            task = self.task_factory.create_finalization_task(
                agent, topic, self.instructions
            )
            ...
            self._status(
                "Cleaning finalization output...",
                step=5,
                detail="Removing leaked instructions",
            )
            logger.info(
                "🧹 Starting Content Cleaning - removing leaked processing instructions"
            )
            ...
            final_post, removed_sections = content_cleaner.clean_finalization_output(
                str(raw_final_post)
            )

            if removed_sections:
                logger.info(
                    f"🧹 Content Cleaning removed {len(removed_sections)} problematic sections:"
                )
                ...
                self._status(
                    ...
                    step=5,
                    detail=f"Removed {len(removed_sections)} instruction leaks",
                )
                ...
                logger.info(
                    "✅ Content Cleaning: No instruction leakage detected - content was clean"
                )
                self._status(
                    "Content cleaning completed",
                    step=5,
                    detail="Content verified clean",
                )
            ...
            processed_post = FlowPostProcessor.process_blog_content(
                content=final_post, topic=topic, force_tool_usage=True
            )
            ...
            if config.features.enable_content_image_injection:
                ...
                final_content = image_injector.ensure_adequate_images(
                    processed_post, topic
                )
            ...
            self._status(
                "Running content integrity validation...",
                step=5,
                detail="Removing hallucinated content",
            )
            logger.info(
                "� Starting Content Integrity Validation - removing hallucinated content and invalid references"
            )
            ...
            integrity_cleaned_content, integrity_report = (
                integrity_validator.validate_content_integrity(final_content)
            )

            # Log integrity validation results
            integrity_summary = format_integrity_report(integrity_report)
            logger.info(
                f"� Content Integrity Validation completed:\n{integrity_summary}"
            )
            ...
            if integrity_report.content_quality_score < 100:
                ...
                logger.info(
                    f"� Content Integrity Validation removed {removed_refs} invalid references and {removed_paragraphs} problematic content sections"
                )
                self._status(
                ...
                logger.info(
                    "✅ Content Integrity Validation: All content verified - no cleanup needed"
                )
                self._status(
                    "Content integrity validation completed",
                    step=5,
                    detail="All content verified successfully",
                )
            ...
            self._status(
                "Running reference deduplication...",
                step=4,
                detail="Consolidating references",
            )
            logger.info(
                "🔧 Starting Reference Deduplication - removing duplicate citations"
            )
            ...
            deduplicated_content, dedup_report = (
                reference_deduplicator.deduplicate_references(final_content)
            )
            ...
            if dedup_report.content_updated:
                logger.info(
                    f"🔧 Reference Deduplication removed {dedup_report.duplicates_removed} duplicate references"
                )
                self._status(
                    "Reference deduplication completed",
                    step=4,
            ...
            logger.info(
                f"   final_content is_empty: {not final_content or not final_content.strip()}"
            )
            logger.info(
                f"   final_content preview: {final_content[:300] if final_content else 'EMPTY FINAL CONTENT'}..."
            )
            ...
            logger.info(
                f"🔍 FLOW FINALIZE - Return dict keys: {list(return_dict.keys())}"
            )
            logger.info(
        ...
        try:
            ...
            openai_tool = next(
                (
                    tool
                    for tool in tools
                    if hasattr(tool, "name") and tool.name == "openai_image_generate"
                ),
                None,
            )
            ...
            if (
                not openai_tool
                or not hasattr(openai_tool, "_api_key")
                or not openai_tool._api_key
            ):
                logger.info(
                    "OpenAI image tool not available, skipping parallel generation"
                )
                return {}
            ...
            with ThreadPoolExecutor(
                max_workers=2
            ) as executor:  # Limit to 2 parallel images
                for concept_name, prompt in image_concepts.items():
                    try:
                        future = executor.submit(
                            self._generate_single_ai_image, openai_tool, prompt
                        )
                        futures[concept_name] = future
                        logger.info(
                            f"Started parallel image generation for: {concept_name}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Failed to start image generation for {concept_name}: {e}"
                        )
    ...
    def _complete_parallel_image_generation(
        self, image_futures: Dict[str, Any]
    ) -> None:
        ...
        for concept_name, future in image_futures.items():
            try:
                ...
                if result and "![" in result:
                    ...
                    logger.warning(
                        f"⚠️ Parallel image generation failed for {concept_name}"
                    )
            except TimeoutError:
                logger.warning(
                    f"⏰ Parallel image generation timed out for {concept_name}"
                )
            except Exception as e:
                logger.warning(
                    f"❌ Parallel image generation error for {concept_name}: {e}"
                )
    ...
    def _generate_image_concepts(self, topic: str) -> Dict[str, str]:
        ...
        # Hero image concept
        concepts["hero"] = (
            f"Professional illustration of {topic}, clean modern style, high-tech aesthetic"
        )
        ...
        if any(
            ...
            concepts["ai_concept"] = (
                "Artificial intelligence brain network, neural connections, futuristic technology"
            )
        elif any(
            word in topic_lower for word in ["data", "analytics", "visualization"]
        ):
            concepts["data_concept"] = (
                "Data visualization charts and graphs, modern dashboard, analytics interface"
            )
        elif any(
            word in topic_lower for word in ["business", "strategy", "management"]
        ):
            concepts["business_concept"] = (
                "Professional business meeting, modern office environment, collaboration"
            )
        elif any(word in topic_lower for word in ["technology", "tech", "software"]):
            concepts["tech_concept"] = (
                "Modern technology interface, clean software design, digital innovation"
            )
    ...
    def _generate_single_ai_image(self, openai_tool, prompt: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"AI image generation failed for prompt '{prompt[:50]}...': {e}"
            )
            return ""
