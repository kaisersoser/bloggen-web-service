...
class CostTracker:
    ...
    def estimate_crew_cost(
        self, crew_result: Any, phase_name: str, agent_count: int = 1
    ):
        """
    ...
    def print_cost_summary(self):
        ...
        if summary.calls_by_model:
            ...
            for model, calls in summary.calls_by_model.items():
                ...
                print(
                    f"   {model}: ${model_cost:.4f} ({model_tokens:,} tokens, {len(calls)} calls)"
                )
        ...
        if summary.calls_by_phase:
            ...
            for phase, calls in summary.calls_by_phase.items():
                ...
                print(
                    f"   {phase}: ${phase_cost:.4f} ({phase_tokens:,} tokens, {len(calls)} calls)"
                )
