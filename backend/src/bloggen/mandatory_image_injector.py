...
class MandatoryImageInjector:
    ...
    def ensure_adequate_images(self, content: str, topic: str) -> str:
        ...
        if existing_images > 3:
            logger.warning(
                f"⚠️ Blog has {existing_images} images (more than recommended 3)"
            )
            return content
        ...
        logger.warning(
            f"❌ Blog has insufficient images ({existing_images}), adding {images_needed} images"
        )

        return self._inject_missing_images(
            content, topic, images_needed, existing_images
        )
    ...
    def _inject_missing_images(
        self, content: str, topic: str, images_needed: int, existing_count: int
    ) -> str:
        ...
        for i in range(images_needed):
            ...
            if image_number == 1:
                ...
                modified_content = self._insert_hero_image(
                    modified_content, image_markdown
                )
            else:
                # Supporting image - mix of Unsplash and AI
                use_ai = i % 2 == 1  # Alternate between Unsplash and AI
                image_markdown = self._generate_supporting_image(
                    topic, image_number, use_ai
                )
                modified_content = self._insert_supporting_image(
                    modified_content, image_markdown, sections
                )
    ...
    def _generate_supporting_image(
        self, topic: str, image_number: int, use_ai: bool = False
    ) -> str:
        ...
        try:
            ...
            if "placeholder" in result.lower() or "placehold" in result.lower():
                logger.warning(
                    f"Supporting image #{image_number} fell back to placeholder"
                )
                return self._create_fallback_image(topic, f"supporting-{image_number}")

            logger.info(
                f"✅ Generated supporting image #{image_number} (Unsplash or AI)"
            )
            return result
    ...
    def _insert_supporting_image(
        self, content: str, image_markdown: str, sections: List[str]
    ) -> str:
