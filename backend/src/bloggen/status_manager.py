...
class StatusUpdateManager:
    ...
    def send_status_update(self, message: str, step: int, detail: Optional[str] = None):
        ...
        if self.status_callback:
            ...
            progress = step_progress_map.get(
                step, min((step / self.total_steps) * 100, 100)
            )
            ...
            try:
                ...
                logger.debug(
                    f"Enhanced status update sent: {message} (Step {step}/{self.total_steps})"
                )
    ...
    def send_agent_thinking(self, agent_name: str, thought: str):
        ...
        logger.info(
            f"🔍 DEBUG StatusUpdateManager.send_agent_thinking called: agent={agent_name}, thought={thought[:50]}..."
        )
        if self.status_callback:
            try:
                ...
                logger.info(
                    f"🔍 DEBUG StatusUpdateManager.send_agent_thinking calling callback with progress={current_progress}"
                )
                self.status_callback(
                    {
                ...
                logger.info(
                    f"🔍 DEBUG StatusUpdateManager.send_agent_thinking callback completed successfully"
                )
            except Exception as e:
                logger.error(f"Failed to send agent thinking update: {e}")
        else:
            logger.warning(
                f"🔍 DEBUG StatusUpdateManager.send_agent_thinking: No status_callback available!"
            )

    def send_tool_usage(
        self, tool_name: str, input_summary: str, agent_name: str = "Unknown"
    ):
    ...
    def send_content_stream(
        self, content_type: str, content: str, is_partial: bool = False
    ):
        ...
        if self.status_callback:
            try:
                ...
                logger.debug(
                    f"Content stream update sent: {content_type} ({len(content)} chars)"
                )
            except Exception as e:
