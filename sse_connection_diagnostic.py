        ...
        async with session.get(stream_url) as response:
            ...
            try:
                async for line in response.content:
                    ...
                    if elapsed > 60:
                        print(f"   ⏰ Test completed after 60s - received {message_count} messages")
                        break
                    ...
                    if line.startswith('data: '):
                        if first_message_time is None:
                            ...
                            first_msg_delay = (first_message_time - connection_start).total_seconds()
                        ...
                        try:
                            ...
                            print(f"   📨 [{elapsed:5.1f}s] Message {message_count}: [{msg_type}] {progress}% - {message}...")
                            
                            # Stop after getting several messages to analyze timing
                            if message_count >= 10:
                                print(f"   ✅ Successfully received {message_count} messages - SSE working!")
                                break
                ...
                if first_message_time:
                    first_delay = (first_message_time - connection_start).total_seconds()
                ...
                if connection_time > 30:
                    print(f"   🚨 ISSUE: Connection took {connection_time:.2f}s (>30s timeout)")
                elif first_message_time is None:
                    print(f"   🚨 ISSUE: No messages received - backend may not be sending SSE data")
