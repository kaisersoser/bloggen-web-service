...
import { PromptConfig } from '@/components/blog/ChatPromptInput';
...
export function useBlogGenerator() {
  ...
  const { jobs, previousBlogs, blogsLoading, updateJob, createJob, deleteJob, fetchPreviousBlogs, deleteBlog } = useBlogManagement();
  ...
  useEffect(() => { if (jobs.length > 0 && !currentJobId) setCurrentJobId(jobs[jobs.length - 1].id); }, [jobs, currentJobId]);
  ...
  const handleGenerateBlog = useCallback(async (topic: string, instructions: string, _config?: PromptConfig) => {
    ...
    try { setGenerationError(null); setIsGenerating(true); completedTasksRef.current.clear(); const data = await blogService.generateBlog(topic.trim(), instructions.trim()); createJob(data.task_id, topic.trim(), instructions.trim()); setCurrentJobId(data.task_id); setActiveConnectionId(data.task_id); try { await connectToTaskStream(data.task_id, (taskId, updates) => updateJob(taskId, updates), handleTaskCompletion, handleTaskError, (taskId, log) => setTaskLogs(prev => ({ ...prev, [taskId]: [...(prev[taskId] || []), log] }))); } catch (sseErr) { console.error('Failed to start SSE stream:', sseErr); setGenerationError('Failed to establish real-time connection. Generation continues in background.'); } } catch (err) { console.error('Error starting blog generation:', err); const msg = err instanceof Error ? err.message : 'Failed to start blog generation.'; setGenerationError(msg); if (activeConnectionId) setActiveConnectionId(null); } finally { setIsGenerating(false); } }, [canGenerate, activeConnectionId, closeConnection, completedTasksRef, createJob, updateJob, connectToTaskStream, handleTaskCompletion, handleTaskError]);
  ...
  const handleDeleteCurrentItem = useCallback(async () => { if (!currentJobId || !currentJob) return; if (activeConnectionId === currentJobId) { closeConnection(); setActiveConnectionId(null); } try { if (currentJob.status === 'completed') { await deleteBlog(currentJobId); } else { deleteJob(currentJobId); } setCurrentJobId(null); setGenerationError(null); } catch (err) { console.error('Delete failed:', err); setGenerationError('Failed to delete. Please retry.'); } }, [currentJobId, currentJob, activeConnectionId, closeConnection, deleteBlog, deleteJob]);
  ...
  const handleNewBlog = useCallback(() => { if (isGenerating && activeConnectionId) return; setCurrentJobId(null); setGenerationError(null); setSelectedBlog(null); setShowBlogModal(false); }, [isGenerating, activeConnectionId]);

  return { isAuthenticated, isLoading, stats, statsLoading, isFree, jobs, previousBlogs, blogsLoading, currentJobId, currentJob, generationError, isGenerating, activeConnectionId, taskLogs, showDeleteDialog, blogToDelete, isDeleting, selectedBlog, showBlogModal, canGenerate, handleGenerateBlog, handleJobClick, handleBlogClick, handleDeleteBlog, handleBulkDeleteBlogs, confirmDeleteBlog, handleNewBlog, handleDeleteCurrentItem, setShowDeleteDialog, setBlogToDelete, setIsDeleting, setGenerationError, setSelectedBlog, setShowBlogModal, refetchStats, fetchPreviousBlogs };
}
